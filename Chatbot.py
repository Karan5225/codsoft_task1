...
class SupportBot:
    ...
    def __init__(self):  # Fixed typo from `_init_` to `__init__`
        self.support_responses = {
            'ask_about_product': r'.*\bproduct\b.*',  # Fixed regex patterns
            'technical_support': r'.*\btechnical.*support\b.*',
            'about_returns': r'.*\breturn.*policy\b.*',
            'general_query': r'.*\bhow.*help\b.*'
        }
    ...
    def greet(self):
        ...
        if will_help.lower() in self.negative_res:  # Ensure case insensitivity
